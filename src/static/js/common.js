...
map.filtration();
let $sliderInfro = $('[data-slider-infro]');
let $sliderInfroPrev = $('[data-slider-infro-prev]');
let $sliderInfroNext = $('[data-slider-infro-next]');
let sliderInfro = new Swiper($sliderInfro, {
    loop: true,
    //loop: true,
    //slidesPerView: 3,
    slidesPerView: 'auto',
    ...
    spaceBetween: 0,
    navigation: {
        nextEl: $sliderInfroNext,
        prevEl: $sliderInfroPrev
    },
    pagination: {
        el: '[data-slider-infro-pagination]',
        type: 'custom',
        renderCustom: function(swiper, current, total) {
            let currentSlide = current.toString();
            let totalSlide = total.toString();
            return `<span class="InfrastructureSlider_current">${currentSlide}</span> РёР· <span class="InfrastructureSlider_total">${totalSlide}</span>`;
        }
    },
    centeredSlides: true
});
